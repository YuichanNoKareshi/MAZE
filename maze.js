    ...
    for( var i = 0; i < cols; i++ ) {
        for( var j = 0; j < rows; j++ ) {
            switch( mazes[index][i][j] ) {
                case 0: ctxs[index].fillStyle = "black"; break;//路
                case 1: ctxs[index].fillStyle = "gray"; break;//不能走的地方
                case 2: ctxs[index].fillStyle = "red"; break;//走过的地方
                case 3: ctxs[index].fillStyle = "yellow"; break;//正确的路
                case 4: ctxs[index].fillStyle = "#500000"; break;
                case 8: ctxs[index].fillStyle = "blue"; break;//起点
                case 9: ctxs[index].fillStyle = "gold"; break;//终点
            }
...
function solveMaze1(index) {
    ...
    if( neighbours.length ) {//如果四面有没走过的路
...
function solveMaze1New(index) {
    ...
    var neighbours = getFNeighboursNew( 1, start[index].x, start[index].y, 0 );//找到偏向终点的没走过的路
...
function solveMaze2New(index){
    ...
    var neighbours = getFNeighboursNew2( 1, start[index].x, start[index].y, 0 );//找到偏向终点的没走过的路
    if( neighbours.length ) {
        stacks[index].push( start[index] );
        start[index] = neighbours[0];
        mazes[index][start[index].x][start[index].y] = 2;
...
function createMaze2(ctx) {
    ...
    if(start[0].x == (cols - 1) && start[0].y == (rows - 1)){
        ...
        document.getElementById("canvas2").addEventListener( "mousedown", getCursorPos2, false);
        document.getElementById("btnCreateMaze").removeAttribute("disabled");
        return;
    }
